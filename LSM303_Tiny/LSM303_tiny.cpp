...
bool LSM303_Tiny::timeoutOccurred()
{
  return did_timeout;
}

void LSM303_Tiny::setTimeout(unsigned int timeout)
{
  io_timeout = timeout;
}

unsigned int LSM303_Tiny::getTimeout()
{
  return io_timeout;
}

void LSM303_Tiny::init(byte device, byte sa0_a)
{  
  _device = device;
  switch (_device)
  {
...
// mode.
void LSM303_Tiny::enableDefault(void)
{
...
void LSM303_Tiny::writeAccReg(byte reg, byte value)
{
  TinyWireM.beginTransmission(acc_address);
  TinyWireM.send(reg);
...
byte LSM303_Tiny::readAccReg(byte reg)
{
  byte value;
  ...
  TinyWireM.send(reg);
  last_status = TinyWireM.endTransmission();
  TinyWireM.requestFrom(acc_address, (byte)1);
...
void LSM303_Tiny::writeMagReg(byte reg, byte value)
{
...
byte LSM303_Tiny::readMagReg(int reg)
{
  byte value;
  ...
  TinyWireM.send(reg);
  last_status = TinyWireM.endTransmission();
  TinyWireM.requestFrom(MAG_ADDRESS, 1);
  value = TinyWireM.receive();
  TinyWireM.endTransmission();
...
void LSM303_Tiny::setMagGain(magGain value)
{
  TinyWireM.beginTransmission(MAG_ADDRESS);
  TinyWireM.send(LSM303_CRB_REG_M);
  TinyWireM.send((byte) value);
  TinyWireM.endTransmission();
}

// Reads the 3 accelerometer channels and stores them in vector a
void LSM303_Tiny::readAcc(void)
{
  TinyWireM.beginTransmission(acc_address);
  // assert the MSB of the address to get the accelerometer 
  // to do slave-transmit subaddress updating.
  TinyWireM.send(LSM303_OUT_X_L_A | (1 << 7)); 
  last_status = TinyWireM.endTransmission();
  TinyWireM.requestFrom(acc_address, (byte)6);
  ...
  byte xla = TinyWireM.receive();
  byte xha = TinyWireM.receive();
  byte yla = TinyWireM.receive();
  byte yha = TinyWireM.receive();
  byte zla = TinyWireM.receive();
  byte zha = TinyWireM.receive();
...
void LSM303_Tiny::readMag(void)
{
  TinyWireM.beginTransmission(MAG_ADDRESS);
  TinyWireM.send(LSM303_OUT_X_H_M);
  last_status = TinyWireM.endTransmission();
  ...
  byte xhm = TinyWireM.receive();
  byte xlm = TinyWireM.receive();
  ...
  if (_device == LSM303DLH_DEVICE)
  {
    ...
    yhm = TinyWireM.receive();
    ylm = TinyWireM.receive();
    zhm = TinyWireM.receive();
    zlm = TinyWireM.receive();
  }
  else
  {
...
// returned.
int LSM303_Tiny::heading(vector from)
{
    // shift and scale
...
void LSM303_Tiny::vector_cross(const vector *a,const vector *b, vector *out)
{
...
float LSM303_Tiny::vector_dot(const vector *a,const vector *b)
{
  return a->x*b->x+a->y*b->y+a->z*b->z;
}

void LSM303_Tiny::vector_normalize(vector *a)
{
...
byte LSM303_Tiny::detectSA0_A(void)
{
  TinyWireM.beginTransmission(ACC_ADDRESS_SA0_A_LOW);
  TinyWireM.send(LSM303_CTRL_REG1_A);
  last_status = TinyWireM.endTransmission();
  TinyWireM.requestFrom(ACC_ADDRESS_SA0_A_LOW, 1);
